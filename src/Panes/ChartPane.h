...
class ProjectFile;
class ChartPane : public AbstractPane, public conf::ConfigAbstract {
...
public:  // singleton
    static std::shared_ptr<ChartPane> Instance() {
        static auto _instance = std::make_shared<ChartPane>();
        return _instance;
    }

public:
    ChartPane();                    // Prevent construction
    ChartPane(const ChartPane&){};  // Prevent construction by copying
