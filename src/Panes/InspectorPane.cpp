    ...
    if (rpPtr) {
        ...
        if (pipe) {
            for (auto i = 0; i < pipe->getCountTextures(); i++) {
                ...
                ImGui::Text("Back");
                ImGui::TextureOverLay(aw, pipe->getBackTexture(i).get(), ImVec4(0, 0, 0, 0), ct::toStr("FBO %u", backId).c_str(), ImVec4(0, 0, 0, 1), ImVec4(0, 0, 0, 0));
                ImGui::Text("Front");
                const auto frontId = pipe->getFrontFboID();
                ImGui::TextureOverLay(
                    aw, pipe->getFrontTexture(i).get(), ImVec4(0, 0, 0, 0), ct::toStr("FBO %u", frontId).c_str(), ImVec4(0, 0, 0, 1), ImVec4(0, 0, 0, 0));
            }
        }
        ImGui::EndGroup();
    }
                ...
                for (auto uni : *arrPtr) {
                    if (uni->glslType == uType::uTypeEnum::U_SAMPLER2D && !uni->bufferShaderName.empty()) {
                        ...
                        if (rrp && rrp->GetPipe() && rrp->GetPipe()->getCountTextures() > 0) {
                            ...
                            ImGui::TextureOverLay(aw,
                                                  rrp->GetPipe()->getFrontTexture(0).get(),
                                                  ImVec4(0, 0, 0, 0),
                                                  ct::toStr("FBO %u", frontId).c_str(),
                                                  ImVec4(0, 0, 0, 1),
                                                  ImVec4(0, 0, 0, 0));
                        }
                        ImGui::EndGroup();
