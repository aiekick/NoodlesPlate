    ...
    if (vOpened && *vOpened) {
        ...
        if (ImGui::Begin(GetName().c_str(), vOpened, flags)) {
            ...
            if (puIsThereSomeShaderErrors && puShowErrors) {
                ...
                InterfacePanes::Instance()->DisplayMessageOfRenderPack(
                    MainBackend::sMainThread, false, codeTree, MainBackend::Instance()->puDisplay_RenderPack, showDebug, true, true);

                if (show3DSpace) {
                    InterfacePanes::Instance()->DisplayMessageOfRenderPack(
                        MainBackend::sMainThread, false, codeTree, MainBackend::Instance()->pu3dAxis_RenderPack, showDebug, true, true);
                    InterfacePanes::Instance()->DisplayMessageOfRenderPack(
                        MainBackend::sMainThread, false, codeTree, MainBackend::Instance()->pu3dGrid_RenderPack, showDebug, true, true);
                }

                if (showMesh) {
                    InterfacePanes::Instance()->DisplayMessageOfRenderPack(
                        MainBackend::sMainThread, false, codeTree, MainBackend::Instance()->puMesh_RenderPack, showDebug, true, true);
                }

                if (showCulling) {
                    InterfacePanes::Instance()->DisplayMessageOfRenderPack(
                        MainBackend::sMainThread, false, codeTree, GizmoSystem::Instance()->GetRenderPack(), showDebug, true, true);
                }
            }
