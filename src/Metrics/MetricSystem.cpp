...
void MetricSystem::DrawTooltip(ImVec2 vPos) {
    ...
    ImGuiWindowFlags flags = ImGuiWindowFlags_NoInputs | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize |
        ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoDocking | ImGuiWindowFlags_Tooltip |
...
void MetricSystem::DrawMemoryInfos(const GpuInfoStruct& vGpuInfoStruct) {
    if (vGpuInfoStruct.memsFound) {
        ...
        if (ImGui::TableNextColumn())
            ImGui::Text("%s", FormatNum(totalMemory * 1000).c_str());

        if (ImGui::TableNextColumn())
            ImGui::Text(" Free memory");
        if (ImGui::TableNextColumn())
            ImGui::Text("%s", FormatNum(freeMemory * 1000).c_str());

        if (ImGui::TableNextColumn())
            ImGui::Text(" Used memory");
        if (ImGui::TableNextColumn())
            ImGui::Text("%s", FormatNum(usedMemory * 1000).c_str());

        if (ImGui::TableNextColumn())
            ImGui::Text(" Load memory");
        if (ImGui::TableNextColumn())
            ImGui::Text("%.2f %%", fMemUtilzation);
    }
}
    ...
    if (vGpuInfoStruct.tachosFound) {
        if (vGpuInfoStruct.tacho > 0) {
            if (ImGui::TableNextColumn())
                ImGui::Text(" Fan");
            if (ImGui::TableNextColumn())
                ImGui::Text("%i ", vGpuInfoStruct.tacho);
        }
    }
}
