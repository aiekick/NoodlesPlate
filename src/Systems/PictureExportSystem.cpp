...
void PictureExportSystem::ExportRenderPackToPictureFile(RenderPackWeak vRP, const std::string& vFilePathName, int vBufferId) {
    auto rpPtr = vRP.lock();
    if (rpPtr) {
        if (!vFilePathName.empty() && vBufferId >= 0) {
            ...
            if (ps.isOk) {
                if (ps.ext == "png") {
                    ...
                    LogVarInfo("Fail to save %s. Format %s not supported", vFilePathName.c_str(), ps.ext.c_str());
                }
            }
        }
    }
...
bool PictureExportSystem::Is_TakeScreenSHotForEachModif_Needed() {
    return puTakePictureFromFBOForEachCodeUpdate || puTakePictureFromScreenForEachCodeUpdate || puAlwaysShowShotButton;
}
...
std::string PictureExportSystem::getXml(const std::string& vOffset, const std::string& vUserDatas) {
    ...
    str += offset + "<TakePictureFromScreenForEachCodeUpdate>" + (puTakePictureFromScreenForEachCodeUpdate ? "true" : "false") +
        "</TakePictureFromScreenForEachCodeUpdate>\n";
