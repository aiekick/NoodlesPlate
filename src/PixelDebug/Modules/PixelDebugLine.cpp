...
void PixelDebugLine::DrawTooltips(RenderPackWeak vRenderPack, float vDisplayQuality, CameraSystem* /*vCamera*/) {
    ...
    if (rpPtr && rpPtr->GetPipe()) {
        ...
        for (auto it = puCapturesPoints.begin(); it != puCapturesPoints.end(); ++it) {
            ...
            if (g.CurrentViewport) {
                if (puGraphHoveredIdx == idx) {
                    ImGui::GetCurrentWindow()->DrawList->AddRectFilled(g.CurrentViewport->Pos + ImVec2(p.x - 5.0f, p.y - 5.0f),
                                                                       g.CurrentViewport->Pos + ImVec2(p.x + 5.0f, p.y + 5.0f),
                                                                       ImGui::GetColorU32(ImVec4(1.0f, 1.0f, 1.0f, 1.0f)));
                } else {
                    ImGui::GetCurrentWindow()->DrawList->AddRectFilled(g.CurrentViewport->Pos + ImVec2(p.x - 2.0f, p.y - 2.0f),
        ...
        if (g.CurrentViewport) {
            const auto p0 = g.CurrentViewport->Pos +
                ImVec2(puFirstPos.x * rpPtr->puMeshRect.w + rpPtr->puMeshRect.x, (1.0f - puFirstPos.y) * rpPtr->puMeshRect.h + rpPtr->puMeshRect.y) * vDisplayQuality;
            const auto p1 = g.CurrentViewport->Pos +
                ImVec2(puLastPos.x * rpPtr->puMeshRect.w + rpPtr->puMeshRect.x, (1.0f - puLastPos.y) * rpPtr->puMeshRect.h + rpPtr->puMeshRect.y) * vDisplayQuality;
...
void PixelDebugLine::DrawPane(RenderPackWeak vRenderPack, float vDisplayQuality, CameraSystem* /*vCamera*/) {
    ...
    if (rpPtr) {
        if (ImGui::CollapsingHeader("Under Line")) {
            if (ImGui::RadioButtonLabeled(ImVec2(0.0f, 0.0f), "Edit##editpoints", "Edit point", puPointToPlace > 0))
                puPointToPlace = 1 - puPointToPlace;
...
void PixelDebugLine::CreateBuffer(RenderPackWeak vRenderPack, ct::fvec2 vStartPos, ct::fvec2 vEndPos) {
    auto rpPtr = vRenderPack.lock();
    if (rpPtr && rpPtr->GetPipe()) {
        if (vStartPos > 0.0f && vEndPos < 1.0f && IS_FLOAT_DIFFERENT(vStartPos.x, vEndPos.x) && IS_FLOAT_DIFFERENT(vStartPos.y, vEndPos.y)) {
            DestroyBuffer();
...
void PixelDebugLine::CaptureFromGPU(RenderPackWeak vRenderPack, int vAttachmentId) {
    ...
    if (rpPtr && rpPtr->GetPipe()) {
        ...
        if (puBuffer && !puBufferSize.emptyAND() && puCapturePos >= 0) {
            if (puCapturePos.x + puBufferSize.x < fboSize.x && puCapturePos.y + puBufferSize.y < fboSize.y) {
...
void PixelDebugLine::DrawGraph(size_t vAttachment, RenderPackWeak vRenderPack, float /*vDisplayQuality*/) {
    ...
    if (vAttachment <= rpPtr->puZeroBasedMaxSliceBufferId) {
        ...
        if (puLimitType == PixelDebugMeasureLimitType::PIXEL_DEBUG_MEASURE_LIMIT_AUTO) {
            ...
            ImGui::InputFloatDefault(70.0f, "##MinY", &puGraphLimits.x, 0.0f);
            ImGui::SameLine();
        ...
        ImGui::EndGroup();
        ImGui::SameLine();
        ImGui::PlotFVec4Histo(
            "##SHOW_CHANNEL", puValuesHisto, puCountPointToShowOnGraph, puShowLine, ImVec2(200, 92), puGraphLimits.x, puGraphLimits.y, &puGraphHoveredIdx);
