            ...
            if (rpPtr->puMeshRect.IsContainPoint(pos)) {
                ...
                for (auto i = 0; i <= rpPtr->puZeroBasedMaxSliceBufferId; i++) {
                    if (puUseAttachment[i]) {
                        ImGui::Text("Att %i (%0.3f, %0.3f, %0.3f, %0.3f)", i, puValues[i].x, puValues[i].y, puValues[i].z, puValues[i].w);
                    }
                }
                ImGui::EndTooltip();
            }
        ...
        if (vAttachment <= rpPtr->puZeroBasedMaxSliceBufferId) {
            ...
            if (puUseAttachment[vAttachment]) {
                ...
                if (puLimitType[vAttachment] == PixelDebugMeasureLimitType::PIXEL_DEBUG_MEASURE_LIMIT_AUTO) {
                    ...
                    ImGui::InputFloatDefault(70.0f, "##MinY", &puGraphLimits[vAttachment].x, 0.0f);
                    ImGui::SameLine();
                ...
                ImGui::EndGroup();
                ImGui::SameLine();
                ImGui::PlotFVec4Histo("##SHOW_CHANNEL",
                                      puValuesHisto[vAttachment],
                                      puCountPointToShowOnGraph[vAttachment],
                                      puShowLine[vAttachment],
                                      ImVec2(200, 92),
                                      puGraphLimits[vAttachment].x,
                                      puGraphLimits[vAttachment].y);
