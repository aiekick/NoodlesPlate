...
namespace
{
    ...
    {
	    auto error = FT_New_Memory_Face(ft_library, (uint8_t*)cfg.FontData, (uint32_t)cfg.FontDataSize, (uint32_t)cfg.FontNo, &Face);
        if (error != 0)
            return false;
    ...
    const FT_Glyph_Metrics* FreeTypeFont::LoadGlyph(uint32_t codepoint)
    {
        ...
        if (glyph_index == 0)
            return nullptr;
	    const auto error = FT_Load_Glyph(Face, glyph_index, LoadFlags);
        if (error)
            return nullptr;
    ...
    const FT_Bitmap* FreeTypeFont::RenderGlyphAndGetInfo(GlyphInfo* out_glyph_info)
    {
	    ...
	    const auto error = FT_Render_Glyph(slot, RenderMode);
        if (error != 0)
            return nullptr;

	    const auto ft_bitmap = &Face->glyph->bitmap;
        ...
        case FT_PIXEL_MODE_BGRA:
            {
                ...
                if (multiply_table == nullptr)
                {
                    for (uint32_t y = 0; y < h; y++, src += src_pitch, dst += dst_pitch)
                        for (uint32_t x = 0; x < w; x++)
                        {
	                        const auto r = src[x * 4 + 2], g = src[x * 4 + 1], b = src[x * 4], a = src[x * 4 + 3];
...
bool ImFontAtlasBuildWithFreeTypeEx(FT_Library ft_library, ImFontAtlas* atlas, unsigned int extra_flags)
{
    ...
    for (auto src_i = 0; src_i < atlas->ConfigData.Size; src_i++)
    {
	    auto& src_tmp = src_tmp_array[src_i];
	    auto& cfg = atlas->ConfigData[src_i];
	    auto& font_face = src_tmp.Font;
        IM_ASSERT(cfg.DstFont && (!cfg.DstFont->IsLoaded() || cfg.DstFont->ContainerAtlas == atlas));

        // Find index from cfg.DstFont (we allow the user to set cfg.DstFont. Also it makes casual debugging nicer than when storing indices)
        src_tmp.DstIndex = -1;
        for (auto output_i = 0; output_i < atlas->Fonts.Size && src_tmp.DstIndex == -1; output_i++)
	    ...
	    auto& dst_tmp = dst_tmp_array[src_tmp.DstIndex];
        src_tmp.SrcRanges = cfg.GlyphRanges ? cfg.GlyphRanges : atlas->GetGlyphRangesDefault();
        for (auto src_range = src_tmp.SrcRanges; src_range[0] && src_range[1]; src_range += 2)
        ...
        for (auto src_range = src_tmp.SrcRanges; src_range[0] && src_range[1]; src_range += 2)
            for (int codepoint = src_range[0]; codepoint <= (int)src_range[1]; codepoint++)
            {
                if (dst_tmp.GlyphsSet.TestBit(codepoint))    // Don't overwrite existing glyphs. We could make this an option (e.g. MergeOverwrite)
                    continue;
                auto glyph_index = FT_Get_Char_Index(src_tmp.Font.Face, codepoint); // It is actually in the font? (FIXME-OPT: We are not storing the glyph_index..)
                if (glyph_index == 0)
                    continue;
    ...
    for (auto dst_i = 0; dst_i < dst_tmp_array.Size; dst_i++)
    ...
    const auto BITMAP_BUFFERS_CHUNK_SIZE = 256 * 1024;
    auto buf_bitmap_current_used_bytes = 0;
    ImVector<unsigned char*> buf_bitmap_buffers;
    buf_bitmap_buffers.push_back((unsigned char*)IM_ALLOC(BITMAP_BUFFERS_CHUNK_SIZE));

    // 4. Gather glyphs sizes so we can pack them in our virtual canvas.
    // 8. Render/rasterize font characters into the texture
    auto total_surface = 0;
    auto buf_rects_out_n = 0;
    for (auto src_i = 0; src_i < src_tmp_array.Size; src_i++)
    {
	    auto& src_tmp = src_tmp_array[src_i];
	    auto& cfg = atlas->ConfigData[src_i];
        ...
        const auto multiply_enabled = (cfg.RasterizerMultiply != 1.0f);
        unsigned char multiply_table[256];
        if (multiply_enabled)
            ImFontAtlasBuildMultiplyCalcLookupTable(multiply_table, cfg.RasterizerMultiply);

        // Gather the sizes of all rectangles we will need to pack
        const auto padding = atlas->TexGlyphPadding;
        for (auto glyph_i = 0; glyph_i < src_tmp.GlyphsList.Size; glyph_i++)
        {
	        auto& src_glyph = src_tmp.GlyphsList[glyph_i];

	        auto metrics = src_tmp.Font.LoadGlyph(src_glyph.Codepoint);
            if (metrics == nullptr)
                continue;

            // Render glyph into a bitmap (currently held by FreeType)
	        auto ft_bitmap = src_tmp.Font.RenderGlyphAndGetInfo(&src_glyph.Info);
            IM_ASSERT(ft_bitmap);

            // Allocate new temporary chunk if needed
            const auto bitmap_size_in_bytes = src_glyph.Info.Width * src_glyph.Info.Height * 4;
    ...
    const auto surface_sqrt = (int)ImSqrt((float)total_surface) + 1;
    atlas->TexHeight = 0;
    ...
    const auto TEX_HEIGHT_MAX = 1024 * 32;
    const auto num_nodes_for_packing_algorithm = atlas->TexWidth - atlas->TexGlyphPadding;
    ...
    for (auto src_i = 0; src_i < src_tmp_array.Size; src_i++)
    {
        ...
        for (auto glyph_i = 0; glyph_i < src_tmp.GlyphsCount; glyph_i++)
    ...
    for (auto src_i = 0; src_i < src_tmp_array.Size; src_i++)
    {
	    ...
	    auto& cfg = atlas->ConfigData[src_i];
	    auto dst_font = cfg.DstFont;

        const auto ascent = src_tmp.Font.Info.Ascender;
        const auto descent = src_tmp.Font.Info.Descender;
        ImFontAtlasBuildSetupFont(atlas, dst_font, &cfg, ascent, descent);
        const auto font_off_x = cfg.GlyphOffset.x;
        const auto font_off_y = cfg.GlyphOffset.y + IM_ROUND(dst_font->Ascent);

        const auto padding = atlas->TexGlyphPadding;
        for (auto glyph_i = 0; glyph_i < src_tmp.GlyphsCount; glyph_i++)
        {
	        auto& src_glyph = src_tmp.GlyphsList[glyph_i];
	        auto& pack_rect = src_tmp.Rects[glyph_i];
            ...
            const auto tx = pack_rect.x + padding;
            const auto ty = pack_rect.y + padding;

            // Blit from temporary buffer to final texture
	        auto blit_src_stride = (size_t)src_glyph.Info.Width;
	        auto blit_dst_stride = (size_t)atlas->TexWidth;
	        auto blit_src = src_glyph.BitmapData;
            if (atlas->TexPixelsAlpha8 != nullptr)
            {
	            auto blit_dst = atlas->TexPixelsAlpha8 + (ty * blit_dst_stride) + tx;
                for (auto y = 0; y < info.Height; y++, blit_dst += blit_dst_stride, blit_src += blit_src_stride)
                    for (auto x = 0; x < info.Width; x++)
                        blit_dst[x] = (unsigned char)((blit_src[x] >> IM_COL32_A_SHIFT) & 0xFF);
            }
            else
            {
	            auto blit_dst = atlas->TexPixelsRGBA32 + (ty * blit_dst_stride) + tx;
                for (auto y = 0; y < info.Height; y++, blit_dst += blit_dst_stride, blit_src += blit_src_stride)
                    for (auto x = 0; x < info.Width; x++)
            ...
            // Register glyph
	        auto x0 = info.OffsetX + font_off_x;
	        auto y0 = info.OffsetY + font_off_y;
	        auto x1 = x0 + info.Width;
	        auto y1 = y0 + info.Height;
	        auto u0 = (tx) / (float)atlas->TexWidth;
	        auto v0 = (ty) / (float)atlas->TexHeight;
	        auto u1 = (tx + info.Width) / (float)atlas->TexWidth;
	        auto v1 = (ty + info.Height) / (float)atlas->TexHeight;
    ...
    // Cleanup
    for (auto buf_i = 0; buf_i < buf_bitmap_buffers.Size; buf_i++)
...
static void* FreeType_Realloc(FT_Memory /*memory*/, long cur_size, long new_size, void* block)
{
    ...
    {
	    const auto new_block = GImGuiFreeTypeAllocFunc((size_t)new_size, GImGuiFreeTypeAllocatorUserData);
...
static bool ImFontAtlasBuildWithFreeType(ImFontAtlas* atlas)
{
    ...
    const auto error = FT_New_Library(&memory_rec, &ft_library);
    if (error != 0)
        return false;
    ...
    const auto ret = ImFontAtlasBuildWithFreeTypeEx(ft_library, atlas, atlas->FontBuilderFlags);
