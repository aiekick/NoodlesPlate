...
void SettingsDlg::DrawPane_General(const SettingsPaneModeEnum& vMode) {
    if (vMode == SettingsPaneModeEnum::SETTINGS_PANE_MODE_LOAD) {
        ...
        if (ImGui::SliderFloatDefaultCompact(
                ImGui::GetContentRegionAvail().x - 50.0f, "Global UI Scale", &StaticConfig::Instance()->puGlobalUIScale, 0.1f, 10.0f, 1.0f, 0.1f)) {
