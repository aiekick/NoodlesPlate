...
FontExplorer::FontExplorer()
{
	...
	// vu que FontExplorer::Instance() risquerait d'etre appelé ici
}
		...
		if (!error)
		{
			if (lib)
			{
				...
				if (!error)
				{
					...
					for (std::string::iterator it = charSetFiletered.begin();
						it != charSetFiletered.end(); ++it)
					{
						...
						if (!error)
						{
							...
							glyph.horiBearing = ct::fvec3(
								face->glyph->metrics.horiBearingX,
								face->glyph->metrics.horiBearingY,
								face->glyph->metrics.horiAdvance) / 64.0f;

							glyph.vertBearing = ct::fvec3(
...
void FontExplorer::ListSystemsFont_Win32()
{
	...
	do {
		...
		result = RegEnumValue(hKey, valueIndex, valueName, &valueNameSize,
			0, &valueType, valueData, &valueDataSize);
...
{
	return m_SystemFontsDataBase.find(vFontName) != 
		m_SystemFontsDataBase.end(); // trouvé
}
		...
		for (std::vector<std::string>::iterator it = lines.begin(); it != lines.end(); ++it)
		{
			...
			if (vec.size() > 1)
			{
				...
				else
				{
					...
					if (vParams->Glyphs.find(c) != vParams->Glyphs.end()) // trouvé
					{
						if (vec.size() > 1)
						{
...
void FontExplorer::AddGlyphInGlyphInversionList(GlyphStruct *vGlyph)
{
	...
	{
		for (std::vector<msdfgen::Contour>::iterator contour = vGlyph->shape.contours.begin();
			contour != vGlyph->shape.contours.end(); ++contour)
		{
