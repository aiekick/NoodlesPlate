...
void Generator::GenerateFullMap(std::atomic<float> &vProgress, std::atomic<bool> &vWorking, std::atomic<GLuint> &vCurrentTexture, std::atomic<float> &vGenerationTime) {
    ...
    if (!m_Params->IsFontLoaded) {
        if (m_Params->FontLoadingMode == FontLoadingModeEnum::FONT_LOADING_FROM_SYSTEM) {
            m_Params->IsFontLoaded = m_FontExplorer->LoadFontFromSystem(m_Params->fontName, m_Params->charSet, m_Params);
        } else if (m_Params->FontLoadingMode == FontLoadingModeEnum::FONT_LOADING_FROM_FILE) {
            m_Params->IsFontLoaded = m_FontExplorer->LoadFontFromFile(m_Params->FontFileLoaded, m_Params->charSet, m_Params);
        }
    }
    ...
    if (m_Params->IsFontLoaded) {
        if (CreateImageBuffer()) {
            ...
            vProgress = 0.0f;
            int index = 0;
            for (auto itGlyphs = m_Params->Glyphs.begin(); itGlyphs != m_Params->Glyphs.end(); ++itGlyphs) {
                ...
                if (glyph->shape.validate()) {
                    ...
                    if (m_Params->algo == AlgoEnum::ALGO_MSDF)
                        msdfgen::edgeColoringSimple(glyph->shape, (double)m_Params->msdfColoringAngle);
            ...
            index = 0;
            for (std::list<BinPackRectStruct>::iterator it = m_Params->m_BinPackRects.begin(); it != m_Params->m_BinPackRects.end(); ++it) {
...
void Generator::UpdateGhars(std::atomic<float> &vProgress, std::atomic<bool> &vWorking, std::atomic<GLuint> &vCurrentTexture, std::atomic<float> &vGenerationTime) {
    GuiBackend::Instance()->MakeContextCurrent(GenerationThread::sGenerationThread);
...
void Generator::InvertGlyph(GlyphStruct *vGlyph) {
    ...
    for (auto itInv = signs->begin(); itInv != signs->end(); ++itInv) {
        bool b = *itInv;
...
bool Generator::CreateImageBuffer() {
    ...
    if (m_Params->texMaxSize.x < 4096 && m_Params->texMaxSize.y < 4096 && m_Params->bytesPerPixel >= 1 && m_Params->bytesPerPixel <= 4) {
        m_Params->m_ImageDataSize = (int)m_Params->texMaxSize.x * (int)m_Params->texMaxSize.y * m_Params->bytesPerPixel;
...
void Generator::Upload1ChannelData(stbrp_coord *vX, stbrp_coord *vY, msdfgen::Bitmap<float> *vBuffer, int vMaxWidth) {
    GuiBackend::Instance()->MakeContextCurrent(GenerationThread::sGenerationThread);
...
void Generator::Upload3ChannelData(stbrp_coord *vX, stbrp_coord *vY, msdfgen::Bitmap<float, 3> *vBuffer, int vMaxWidth) {
    GuiBackend::Instance()->MakeContextCurrent(GenerationThread::sGenerationThread);
...
void Generator::CreateAndUploadFontTexture(ct::ivec2 vSize, bool vJustUpdate) {
    ...
    if (m_Params->m_ImageData != 0 && m_Params->m_ImageDataSize > 0) {
        ...
        if (m_Params->bytesPerPixel == 3) {
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, (GLsizei)vSize.x, (GLsizei)vSize.y, 0, GL_RGB, GL_UNSIGNED_BYTE, m_Params->m_ImageData);

            m_Params->textureSize = vSize;
        } else if (m_Params->bytesPerPixel == 4) {
            glPixelStorei(GL_UNPACK_ALIGNMENT, 1);

            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, (GLsizei)vSize.x, (GLsizei)vSize.y, 0, GL_RGBA, GL_UNSIGNED_BYTE, m_Params->m_ImageData);

            m_Params->textureSize = vSize;
        } else if (m_Params->bytesPerPixel == 1) {
            glPixelStorei(GL_UNPACK_ALIGNMENT, 1);

            glTexImage2D(GL_TEXTURE_2D, 0, GL_ALPHA, (GLsizei)vSize.x, (GLsizei)vSize.y, 0, GL_ALPHA, GL_UNSIGNED_BYTE, m_Params->m_ImageData);
...
void Generator::UpdateFontTexture(ct::fvec2 vOffset, ct::fvec2 vSize) {
    ...
    if (m_Params->m_ImageData != 0 && m_Params->m_ImageDataSize > 0 && m_Params->textureFont > 0) {
        ...
        if (m_Params->bytesPerPixel == 3) {
            ...
            glTexSubImage2D(
                GL_TEXTURE_2D, 0, (GLsizei)vOffset.x, (GLsizei)vOffset.y, (GLsizei)vSize.x, (GLsizei)vSize.y, GL_RGB, GL_UNSIGNED_BYTE, m_Params->m_ImageData);
        } else if (m_Params->bytesPerPixel == 4) {
            glTexSubImage2D(
                GL_TEXTURE_2D, 0, (GLsizei)vOffset.x, (GLsizei)vOffset.y, (GLsizei)vSize.x, (GLsizei)vSize.y, GL_RGBA, GL_UNSIGNED_BYTE, m_Params->m_ImageData);
        } else if (m_Params->bytesPerPixel == 1) {
            glPixelStorei(GL_UNPACK_ALIGNMENT, 1);

            glTexSubImage2D(
                GL_TEXTURE_2D, 0, (GLsizei)vOffset.x, (GLsizei)vOffset.y, (GLsizei)vSize.x, (GLsizei)vSize.y, GL_ALPHA, GL_UNSIGNED_BYTE, m_Params->m_ImageData);
        }
