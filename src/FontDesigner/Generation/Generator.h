...
public:
    virtual void GenerateFullMap(std::atomic<float> &vProgress, std::atomic<bool> &vWorking, std::atomic<GLuint> &vCurrentTexture, std::atomic<float> &vGenerationTime);
    virtual void UpdateGhars(std::atomic<float> &vProgress, std::atomic<bool> &vWorking, std::atomic<GLuint> &vCurrentTexture, std::atomic<float> &vGenerationTime);
...
private:  // texture methods
    ...
    void Upload1ChannelData(stbrp_coord *vX, stbrp_coord *vY, msdfgen::Bitmap<float> *vBuffer, int vMaxWidth);
    void Upload3ChannelData(stbrp_coord *vX, stbrp_coord *vY, msdfgen::Bitmap<float, 3> *vBuffer, int vMaxWidth);
